...
function startInterval() {
    intervalId = setInterval(get_Status_RFID, 200);
}
...
// Event listener when the page is unloaded (e.g., when navigating away)
window.onunload = function() {
    stopInterval();
};

async function get_Status_RFID() {
    try {
        const response = await fetch(mySecrets().raspberry + "/get_rfid_status", {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
